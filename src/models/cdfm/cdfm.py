...
from src.models.cdfm.utils import  *
from src.models.cdfm.modules import UNet_conditional, EMA
import logging
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, (labels , images) in enumerate(pbar):
            images = images.to(device).type(torch.float)
        ...
        if epoch % 10 == 0:
            labels = torch.rand(14,1).to(device).type(torch.float)
